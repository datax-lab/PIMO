...
for i, (seed1, seed2) in enumerate(zip(random_seeds1, random_seeds2)):
    ...
    train_G_s = train_G_p.transpose(train_G_p.shape[1], train_G_p.shape[0], train_G_p.shape[2])
    np.save(f"{SPLIT_DATA_PATH}/exp_{i+1}_train_rna_pcas" + str(q) + ".npy", train_G_s)
    
    val_G_s   = val_G_p.transpose(val_G_p.shape[1], val_G_p.shape[0], val_G_p.shape[2])
    test_G_s  = test_G_p.transpose(test_G_p.shape[1], test_G_p.shape[0], test_G_p.shape[2])
    ...
    train_M_s = train_M_p.transpose(train_M_p.shape[1], train_M_p.shape[0], train_M_p.shape[2])
    np.save(f"{SPLIT_DATA_PATH}/exp_{i+1}_train_dna_pcas" + str(q) + ".npy", train_M_s)
    
    val_M_s = val_M_p.transpose(val_M_p.shape[1], val_M_p.shape[0], val_M_p.shape[2])
    test_M_s = test_M_p.transpose(test_M_p.shape[1], test_M_p.shape[0], test_M_p.shape[2])
    ...
    train_C_s = train_C_p.transpose(train_C_p.shape[1], train_C_p.shape[0], train_C_p.shape[2])
    np.save(f"{SPLIT_DATA_PATH}/exp_{i+1}_train_cna_pcas" + str(q) + ".npy", train_C_s)
    
    val_C_s   = val_C_p.transpose(val_C_p.shape[1], val_C_p.shape[0], val_C_p.shape[2])
    test_C_s  = test_C_p.transpose(test_C_p.shape[1], test_C_p.shape[0], test_C_p.shape[2])
